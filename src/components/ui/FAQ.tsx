...
const faqItems: FAQItem[] = [
  {
    ...
    icon: <CreditCard className="w-4 h-4 text-brand-cyan" />,
    answer: (
  ...
  },
  {
    ...
    icon: <Zap className="w-4 h-4 text-brand-purple" />,
    answer: (
      <div className="space-y-2">
    ...
    <div className="space-y-3">
      ...
      {faqItems.map((item, index) => (
        <div key={index} className="border border-slate-700 rounded-lg overflow-hidden">
          ...
          {openIndex === index && (
            <div className="px-4 pb-4 pl-11 text-sm text-slate-300 leading-relaxed">
              {item.answer}
            </div>
          )}
        </div>
